...
class SQLAlchemyInstrumentation(BaseInstrumentation):
            ...
            # Use the standard SQLAlchemy event API
                ...
                def before_execute(conn, cursor, stmt, params, context, executemany):
                    try:
                        ...
                        # Store the original statement, parameters and metadata for debugging
                        context._stmt = stmt
                        context._params = params
                        context._engine_metadata = getattr(engine, '_profiler_metadata', {})
                ...
                def after_execute(conn, cursor, stmt, params, context, executemany):
                    try:
                        duration = time.perf_counter() - getattr(context, '_query_start', 0)
                        
                        # Use the stored statement if available, otherwise use the provided one
                        ...
                        query_type = getattr(context, '_query_type', cls._detect_query_type(statement))
                        if query_type:
                        ...
                        if duration > 0.1:  # TODO: Move to configurable threshold
                            print(f"FastAPI Profiler: Tracked slow SQL query ({duration:.4f}s): {statement[:100]}...")
                            print(f"  Query type: {query_type}, Database: {metadata.get('dialect', 'unknown')}")
                    except Exception as e:
                ...
                try:
                    ...
                    def _before_execute_wrapper(conn, cursor, stmt, params, context, executemany):
                        return before_execute(conn, cursor, stmt, params, context, executemany)

                    @event.listens_for(engine, "after_cursor_execute")
                    def _after_execute_wrapper(conn, cursor, stmt, params, context, executemany):
                        return after_execute(conn, cursor, stmt, params, context, executemany)
                    
                    print(f"FastAPI Profiler: Successfully registered SQLAlchemy event listeners")
                except Exception as e:
                    print(f"FastAPI Profiler: Error registering SQLAlchemy event listeners: {str(e)}")
    ...
    def uninstrument(cls, engine: Any) -> None:
        ...
        if engine_id not in cls._instrumented_engines:
            print(f"FastAPI Profiler: Engine {engine} not instrumented, skipping uninstrumentation")
            return
        ...
        try:
            ...
            if hasattr(engine, 'dialect') and hasattr(engine.dialect, 'name'):
                ...
                if hasattr(engine, 'url'):
                    url = str(engine.url)
                    # Try to extract database name from URL safely without revealing credentials
                    if '/' in url:
                        db_name = url.split('/')[-1]
    ...
    def _detect_query_type(statement: str) -> Optional[str]:
        ...
        # TODO: replace with sqlparse or move to rustcore, N.B. this is a simple implementation
        if not statement:
            return None
